...
async function createAndPushRepo(projectPath, repoName, isPrivate = true, onLog = () => {}) {
  ...
  try {
    // 0. Vérifier si le repo existe déjà
    try {
      await execPromise(`gh repo view ${name} --json url`, { cwd: projectPath });
      return {
        success: false,
        repoUrl: null,
        error: `Le repo GitHub "${name}" existe déjà sur votre compte. Choisissez un autre nom dans l'onglet Config ou supprimez-le sur GitHub.`
      };
    } catch (viewErr) {
      // Repo n'existe pas, on continue
    }
    ...
    try {
      ...
      let gitEmail = ghEmail.trim();
      const gitName = ghName.trim() || ghUser; // fallback sur le login si name vide
      if (!gitEmail) {
        // Fallback no-reply si l'email principal est vide (profil privé)
        gitEmail = `${ghUser}@users.noreply.github.com`;
      }
    ...
    if (msg.includes('Author identity unknown') || msg.includes('user.email') || msg.includes('user.name')) {
      friendly = 'Votre identité Git (nom + email) n\'est pas configurée. Allez dans l\'onglet Config > Prérequis, puis cliquez sur "Configurer avec GitHub".';
    }
    onLog(`❌ Error: ${friendly}`);
    return { success: false, repoUrl: null, error: friendly };
  }
}
