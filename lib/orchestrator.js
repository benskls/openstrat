...
const { validateProjectPath, checkGitIdentity, configureGitIdentityFromGitHub } = require('./setup-utils');
const fs = require('fs');
...
async function fullSetup(projectPath, options = {}) {
  ...
  // Vérifier l'identité Git (globale ou locale)
  let gitId = await checkGitIdentity();
  if (!gitId.configured) {
    onLog('⚠️ Identité Git non configurée. Tentative de configuration automatique via GitHub...');
    const autoConfig = await configureGitIdentityFromGitHub();
    if (!autoConfig.success) {
      return {
        success: false,
        steps,
        error: 'Votre identité Git (nom + email) n\'est pas configurée. Allez dans l\'onglet Config > Prérequis, puis cliquez sur "Configurer avec GitHub".'
      };
    }
    onLog(`✅ Identité Git configurée : ${autoConfig.name} <${autoConfig.email}>`);
  } else {
    onLog(`✅ Identité Git OK : ${gitId.name} <${gitId.email}>`);
  }
