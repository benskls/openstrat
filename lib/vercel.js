    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== '.git' && entry.name !== 'node_modules' && !entry.name.startsWith('.')) {
        ...
        if (fs.existsSync(subIndex)) {
          deployDir = path.join(projectPath, entry.name);
          onLog(`📁 Found index.html in ${entry.name}/ — deploying from subfolder`);
          break;
        }
        ...
        for (const subEntry of subEntries) {
          if (subEntry.isDirectory()) {
            ...
            if (fs.existsSync(deepIndex)) {
              deployDir = path.join(projectPath, entry.name, subEntry.name);
              onLog(`📁 Found index.html in ${entry.name}/${subEntry.name}/ — deploying from subfolder`);
              break;
            }
          }
        }
        if (deployDir !== projectPath) break;
      }
    }
  }

  // Nettoyer le lien Vercel existant pour éviter les conflits avec un projet supprimé
  const vercelDir = path.join(deployDir, '.vercel');
  if (fs.existsSync(vercelDir)) {
    onLog('🧹 Cleaning old Vercel project link...');
    fs.rmSync(vercelDir, { recursive: true, force: true });
  }

  try {
    // Déployer depuis le bon dossier (racine ou sous-dossier avec index.html)
    const { stdout } = await execStream('vercel', ['--yes'], { cwd: deployDir }, onLog);
    ...
    let projectUrl = null;
    try {
      // Le projet peut avoir le nom du dossier parent ou du sous-dossier
      const deployName = slugifyVercelName(path.basename(deployDir));
      const namesToTry = [deployName, projectName];
      for (const name of namesToTry) {
        try {
          const { stdout: listOut } = await execPromise(`vercel ls ${name}`, { cwd: projectPath });
          const projectMatch = listOut.match(/https:\/\/vercel\.com\/[^\s]+/);
          if (projectMatch) {
            projectUrl = projectMatch[0];
            break;
          }
        } catch {
          // Try next name
