...
async function getInstallInstructions(tool) {
  ...
  if (tool === 'gh') {
    ...
    if (isMac) {
      ...
      if (hasBrew) {
        ...
        steps.push({
          ...
          note: 'Homebrew n\'est pas détecté. Téléchargez le .pkg depuis la documentation officielle (lien ci-dessous).'
        });
      }
    } else if (isWin) {
    ...
    steps.push({ text: 'Connectez-vous', command: 'gh auth login' });
  ...
  if (tool === 'vercel') {
    ...
    steps.push({ text: 'Connectez-vous', command: 'vercel login' });
    steps.push({ text: 'Revenez ici et cliquez "Vérifier"' });
