...
async function checkCommand(cmd) {
  ...
  // 2. Fallback : utiliser le shell de l'utilisateur (zsh, bash, etc.)
  // charge .zshrc / .bashrc / .bash_profile et trouve les outils récemment installés
  const userShell = process.env.SHELL || '/bin/bash';
  const inShell = await new Promise((resolve) => {
    const shellCmd = process.platform === 'win32'
      ? `where ${cmd}`
      : `${userShell} -ilc 'command -v ${cmd} 2>/dev/null'`;
    exec(shellCmd, (err, stdout) => {
      // Vérifier que stdout contient un chemin valide (pas juste un message de login)
      const output = stdout ? stdout.trim() : '';
      const isValid = !err && output.length > 0 && output.includes(cmd);
      resolve(isValid);
    });
  });
  if (inShell) return true;

  // 3. Fallback ultime : chemins absolus usuels
  const commonPaths = [
    `/usr/local/bin/${cmd}`,
...
/**
 * Vérifie que git a une identité globale configurée (user.name + user.email).
 * @returns {Promise<{configured: boolean, name: string|null, email: string|null}>}
 */
async function checkGitIdentity() {
  try {
    const { stdout: name } = await execPromise('git config --global user.name');
    const { stdout: email } = await execPromise('git config --global user.email');
    return { configured: !!(name.trim() && email.trim()), name: name.trim() || null, email: email.trim() || null };
  } catch (err) {
...
/**
 * Configure l'identité Git globale en récupérant les infos du compte GitHub connecté (gh).
 * @returns {Promise<{success: boolean, name: string|null, email: string|null, error: string|null}>}
 */
async function configureGitIdentityFromGitHub() {
  try {
    const { stdout: ghEmailRaw } = await execPromise('gh api user -q .email');
    const { stdout: ghNameRaw } = await execPromise('gh api user -q .name');
    const { stdout: ghLoginRaw } = await execPromise('gh api user -q .login');
    const ghEmail = ghEmailRaw.trim();
    const ghName = ghNameRaw.trim();
    const ghLogin = ghLoginRaw.trim();
    const name = ghName || ghLogin;
    const email = ghEmail || `${ghLogin}@users.noreply.github.com`;
    if (!name || !email) {
      throw new Error('Impossible de récupérer les informations de profil GitHub.');
    }
    await execPromise(`git config --global user.name "${name}"`);
    await execPromise(`git config --global user.email "${email}"`);
    return { success: true, name, email, error: null };
  } catch (err) {
    return { success: false, name: null, email: null, error: err.stderr || err.message };
  }
}
...
module.exports = {
  ...
  checkGitIdentity,
  configureGitIdentityFromGitHub
};
