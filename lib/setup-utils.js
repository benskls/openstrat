...
async function checkCommand(cmd) {
  ...
  // 2. Fallback : lancer which dans un shell interactif (charge .zshrc/.bashrc)
  // Cela permet de détecter les outils installés après le démarrage du serveur
  const inShell = await new Promise((resolve) => {
    const shell = process.platform === 'win32' ? 'cmd' : 'bash';
    const shellCmd = process.platform === 'win32'
      ? `where ${cmd}`
      : `bash -ilc 'which ${cmd}'`;
    exec(shellCmd, (err) => resolve(!err));
  });
